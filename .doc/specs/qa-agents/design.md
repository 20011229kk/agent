...
**进程边界已于 2026-09-21 实测定位**（`docs/probe-evidence/task3/task3-findings.md`）：

- agent 的 shell 命令是 `Kiro Helper`(pid 3612) → `Kiro`(3590) 的后代进程，无额外隔离
- 委派子代理执行 `fs_write` 期间，Kiro 进程树内**未出现存活 ≳100ms 的新进程**
  （0.1s 间隔 148 次采样；检测下限经金丝雀标定：100ms 可检出、20ms 会漏）
- 即 **IDE 内不存在 per-agent 的操作系统进程边界**，"只沙箱化 `qa-executor`"没有挂载点

在此基础上，Task 3 的结论修正为：**拆分执行单元必须发生在进入 IDE 之前**（独立会话 /
独立运行器），不可能在委派链内部实现。在隔离方案落地并重新验证之前：

- **`qa-lead → qa-executor` 的委派 SHALL NOT 被视为默认安全边界。** 已实测：不存在可供
  操作系统隔离挂载的 per-agent 进程边界，因此该边界不能靠"给子代理配 deny"或"给子代理
  单独加沙箱"建立。
- **安全关键的角色分离改为结构性实现：** 需要其写入范围被真正强制的角色，SHALL 以
  **顶层会话直接调用**（已验证 deny 在直接调用路径生效），SHALL NOT 依赖父代理委派。
...
### 4.6 执行隔离（`isolation/`，2026-09-21 落地并验证）

针对 item 5（shell 子进程写入绕过能力层）。设计前提是 Task 3 第一步的进程边界实测：
shell 命令跑在我们能控制的子进程里，所以这条路径**可以**用容器边界约束；而工具级
`fs_write` 在 IDE 宿主进程内执行，不经过这里（item 8，另行处理）。

| 构件 | 作用 |
|---|---|
| `isolation/Containerfile` | 执行镜像：`python:3.12-slim` + 锁版本 `pytest==8.4.2`、`PyYAML==6.0.2`；依赖在**构建期**装好 |
| `isolation/run-isolated.sh` | 执行包装器：整个仓库 `:ro` 挂载，只有 `--rw <相对路径>` 显式列出的目录可写，`--network=none` |
| `isolation/verify-isolation.sh` | 边界验证：5 条检查，每条要求"命令确实执行 + 拒绝原文 + 宿主机 canary 前后哈希" |

**验证结果（`docs/probe-evidence/task3/results/`）：** 5/5 PASS，含阳性对照。
- C1 阳性对照：可写挂载内写入成功，宿主机可见（证明测量手段有效）
- C2 只读挂载内写入：`Read-only file system`，canary 哈希未变
- C3 未挂载的宿主机路径：容器内**不存在**（靠不存在，不靠权限判定）
- C4 执行期外网：`Network is unreachable`
- C5 仓库根：连 `VERSION` 也写不动
- 另：容器内跑完仓库全量测试 **234 项通过**——隔离没有把活干不了

**依赖版本策略：** 执行期 `--network=none` 意味着不能在执行期装依赖，必须构建期锁定。
这是刻意的：把"临时 pip install 一下"这条既扩大攻击面又破坏可复现性的路关掉。

**未闭环的强制点（不要当成已解决）：** 上述只证明"**如果**命令通过包装器执行，边界成立"。
当前没有任何机制强制 `qa-executor` 必须用这个包装器——它同样可以直接在宿主机上跑命令。
候选强制路径是把该角色的 shell allow 清单收窄到只有 `isolation/run-isolated.sh`（命令层
规则已由 item 6 验证生效：直接/`&&`/`;`/管道被拒），但 item 6 同时发现 `bash -c` 与变量
间接会进入 `ask`——headless/子代理下 `ask` 等同 deny，所以这条路**看起来**可行，
**但必须实测**，不得假设。在实测通过前，隔离只算"可用"，不算"已强制"。

**踩过的两个坑（都会伪装成"隔离生效"，写在这里避免重犯）：**

1. `run-isolated.sh` 最初没有可执行位，5 条命令全是 `permission denied`，而 C2 当时只看
   "canary 哈希未变"就报 PASS。**凡"没发生坏事"型判据，必须附带"坏事确实被尝试过"的
   证据**，否则一律作废。判据已改为强制要求拒绝原文。
2. zsh 会把 `"$WORKDIR:ro"` 解析成历史修饰符 `${WORKDIR:r}` 再接字面 `o`，挂载点静默
   变成 `/worko`：容器照样启动，但**什么都没挂上**。若当时镜像里恰好存在 `/work`，
   写入会落进容器临时层、随容器一起消失，结果静默丢失。挂载串里的变量必须加花括号。
