...
- 进程边界已于 2026-09-21 实测定位（证据：`docs/probe-evidence/task3/task3-findings.md`）：
  委派子代理执行 `fs_write` 期间，Kiro 进程树内不出现存活 ≳100ms 的新进程
  （检测下限经金丝雀标定，20ms 会漏检，故该否定结论 SHALL 限定在 ≳100ms 精度内）。
  因此系统 SHALL NOT 设计为"在委派链内部拆分执行单元"——IDE 内没有 per-agent 进程
  边界可供隔离挂载；需要强制写入范围的角色 SHALL 以顶层会话直接调用（该路径下 deny
  已验证生效），SHALL NOT 依赖父代理委派建立安全边界。
