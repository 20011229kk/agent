...
| `task3/results/isolation-image-build.txt` | `qa-executor:0.1.0` 镜像构建记录（依赖锁版本） | 镜像构建时 |
| `task3/results/isolation-verify-arm64.txt` | **隔离边界验证 5/5 PASS**（含阳性对照与拒绝原文） | `isolation/` 当前状态 |
| `task3/results/isolation-pytest-in-container.txt` | 容器内跑完仓库全量测试 234 项通过 | 同上 |
