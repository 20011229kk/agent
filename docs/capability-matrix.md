...
**2026-09-21 补充与二次收窄。** 上述"同一宿主进程"此前是推断；本轮做了进程创建观测，
但外部复核指出观测结论一度被写成了架构断言，现已收窄。详见
`docs/probe-evidence/task3/task3-findings.md`：

- agent 的 shell 命令是 `Kiro Helper`(pid 3612) → `Kiro`(3590) 的后代进程（已验证）
- 委派子代理执行 `fs_write` 期间，pass 4 的 248 次快照（**实测**相邻间隔
  min 0.095 / p50 0.100 / max 0.105s）**没有记录到** Kiro 树中的新建进程；唯一出现的 `sh`
  与探针自身执行 shell 命令同时刻，归因于测量自身
- 检测下限按每寿命重复 5 次标定：1s/300ms/150ms/100ms 均 5/5，**50ms 为 3/5、20ms 为 1/5**
- **以上不构成 PID 归属。** 采样器只观察进程创建；没有新进程仍可能由采样开始前就存在的
  常驻进程、进程池或服务完成写入。因此**不得**据此断言"所有角色共享同一写入进程"、
  "不存在 per-agent 进程边界"或"委派链内部拆分不可能"——这三条推论**已撤回**
- 早期两轮的间隔表述有误：pass 2 写的"0.1s 间隔"实际约 150ms（148 次 / 22.1 秒），
  pass 3 修法有缺陷（p50 空转到 29ms，同时出现 54.3s 长间隙）。采样器已改为跳过错过的
  节拍，并强制公布实测间隔的 min/p50/p95/**max**——能漏多长的进程由 max 间隙决定
...
| Task 3（2026-09-21 实测 + 外部复核后收窄） | shell 侧（item 5）的容器挂载边界已落地并通过 C0–C9 十项验证（含两条实测绕过被拒）。进程侧只得到"本轮快照没有记录到新建进程"，**PID 归属未取得**，原先"不存在 per-agent 进程边界""委派链内部拆分不可能"两条推论已撤回。item 8 的临时缓解仍是"安全关键角色以顶层会话直接调用"，但其依据是 item 8 自身的权限行为观测，**不是**进程架构结论 |
